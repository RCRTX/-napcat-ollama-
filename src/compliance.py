...
class AIReviewer:
    ...
    def review_messages(self, messages: List[Dict[str, Any]]) -> List[Dict]:
        ...
        Returns:
            违规消息列表
        """
        messages = [msg for msg in messages if self._extract_review_text(msg)]
        if not messages:
            self._last_result = {
...
要求：
1. 不要因为脏话、玩笑、普通闲聊、图片/表情/语音/视频占位本身就确认违规。
2. 如果证据明确，status=confirmed。
3. 如果有风险但证据不够完整，status=suspected。
4. 如果更像误报，status=likely_false_positive。
5. 必须给出标准category，且只能从下列值选择：
{category_list}
...
{{
    ...
    "content_preview": "证据片段或内容摘要"
        ...
        category = self._normalize_category(data.get("category"), violation.get("violation_type", ""))
