...
- **违规分类与二次复核**: 自动标注违规分类，并对AI命中的内容进行二次复核，降低误报
...
| save_images | 是否保存聊天图片到本地缓存 | true |
| max_image_size_mb | 单张图片最大保存大小(MB) | 5 |
| save_files | 是否保存群聊文件/视频/语音附件 | true |
| large_file_confirm_mb | 超过该大小的文件需要确认后保存(MB) | 1024 |
| max_alert_records | 告警文件最多保留记录数 | 1000 |

图片不会因为清理旧文件而被删除。系统会下载/复制后计算图片内容 SHA256 和实际字节数，用“内容哈希+字节数”判断是否为同一张图；即使同一张图换了URL，也会复用已有文件，避免重复占用空间。新图片文件名已简化为 `日期_时间_QQ号_图序号_短哈希_大小B.扩展名`，目录会按 `附件/图片/日期` 分类保存，聊天记录TXT导出的图片也使用同样的去重和分类方式。清理功能只会压缩/删除过旧的JSON聊天数据文件，并压缩告警文件到最近记录。

文件、视频、语音附件会按 `附件/文件/日期`、`附件/视频/日期`、`附件/语音/日期` 分类保存。超过 `large_file_confirm_mb` 的大文件不会立即落盘，系统会先发送确认通知；可以在Web面板“系统配置”的“待确认大文件”中确认/拒绝，也可以私聊机器人回复 `保存 文件ID` 或 `拒绝 文件ID`。
...
| context_before_messages | 每条目标消息审查时携带的上文消息数 | 3 |
| context_after_messages | 每条目标消息审查时携带的下文消息数 | 2 |

AI审查会输出标准分类：诈骗广告/引流推广、违法交易、色情低俗、暴力威胁、人身攻击、隐私泄露、未成年人风险、政治敏感、赌博博彩、恶意刷屏、其他风险。审查每条目标消息时会携带可自定义数量的上下文消息，用来识别拆分广告、连续引流、上下文人身攻击、前后文隐私泄露、前后呼应违法交易等多条消息关联风险。二次复核会把结果标记为“确认违规”“疑似违规”“可能误报”或“二次复核不可用”。二次复核不可用时，系统会保留首次AI结果并继续正常记录和通知，只影响上报依据的标注；可能误报会保留记录但默认不发送通知。

### 二次审核AI配置

二次审核AI使用独立配置，不再绑定主AI模型。可以让主AI使用速度更快的模型，让二次审核使用更强、更谨慎的模型。

| 配置项 | 说明 | 默认值 |
|--------|------|--------|
| enabled | 是否启用二次AI审核 | true |
| api_base | 二次审核API地址 | http://localhost:11434/v1 |
| api_key | 二次审核API Key | ollama |
| model | 二次审核模型名称 | qwen2.5 |
| system_prompt | 二次审核提示词 | 低误报复核提示词 |
...
- **聊天记录**: 按群组/关键词/用户/时间范围查询所有聊天记录，违规消息会直接显示违规分类和二次复核状态
- **违规告警**: 查看所有违规记录，默认按时间从新到旧排列，可按级别、违规分类、二次复核状态和用户筛选
- **群组管理**: 查看各群消息统计和可查日期
- **系统配置**: 在网页端修改监控QQ群、QQ私聊通知开关、告警接收QQ号和聊天记录TXT目录
- **AI设置**: 在网页端修改主审查模型、提示词和审查间隔
- **二次审核AI**: 在网页端单独修改二次审核模型、API地址、API Key、提示词和开关

系统配置页修改后会立即写入 `config/config.json`，并更新当前运行中的监控群、QQ通知接收人和人类可读聊天记录TXT导出目录。聊天记录TXT目录支持在网页端选择文件夹、立即同步保存当前历史记录，也支持一键把旧目录内容转移到新目录。机器人登录账号需要在NapCat侧切换，网页端配置的是“接收告警的QQ号”和“需要监听的QQ群号”。
