...
图片不会因为清理旧文件而被删除。同一个图片URL会使用稳定哈希去重，重复图片不会重复占用空间。新图片会使用易读文件名，包含日期、时间、QQ号、昵称、消息ID、图片序号和短哈希；目录会按 `附件/图片/日期` 分类保存，聊天记录TXT导出的图片也使用同样的分类方式。清理功能只会压缩/删除过旧的JSON聊天数据文件，并压缩告警文件到最近记录。
